...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
        ...
        if self.path.__eq__('/') or self.path.startswith('/?'):
            ...
            if self.path.startswith('/?'):
                ...
                if dict.get('img') != None:
                    img = dict['img']
                if 'color' in dict:
                    color = [int(x) for x in dict['color'].split(',')]
                    img_proc.changeColor(img, (300,100), color, None)
    ...
    def list_directory(self, path, img):
        ...
        f.write(b"<h1 style='text-align:center;'>Wall Paint Visualizer</h1><hr>")
        ...
        f.write(b"</div>")
        f.write(b"<div class='inline' style='display:flex; align-text:center;'>")
